...
async function importData() {
    ...
    console.log(`Expected total rows in Excel: ${totalRows}`);

    console.log("Step 2: Connecting to DB...");
    const client = await pool.connect();
    try {
        // Skip Truncate if already empty?
        const countRes = await client.query('SELECT count(*) FROM psip');
        const count = parseInt(countRes.rows[0].count);
        console.log(`Current rows in table: ${count}`);

        if (count > 0) {
            console.log("Truncating table...");
            await client.query('DELETE FROM psip'); // Use DELETE instead of TRUNCATE
            console.log("Table cleared.");
        } else {
            console.log("Table is empty, skipping clear.");
        }

        const BATCH_SIZE = 1000;
        let inserted = 0;

        console.log("Step 3: Starting Import Loop...");
        ...
        for (let r = 1; r < totalRows; r += BATCH_SIZE) {
            ...
            console.log(`Inserted range ${r}-${endRow}. Total: ${inserted}`);
        }
