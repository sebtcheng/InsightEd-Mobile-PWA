  ...
  useEffect(() => {
    const fetchUserAndProjects = async () => {
      try {
        ...
        if (!user) {
           // Wait or redirect? For now just return
           return;
        }

        // 1. Get User Details (for role/municipality check if needed)
        // For now, we just use UID to fetch projects or depend on backend filtering
        const resUser = await fetch(`${API_BASE}/api/user-info/${user.uid}`);
        const dataUser = await resUser.json();
        setUserData(dataUser);

        // 2. Fetch Projects
        // Passing UID to filter by creator (LGU)
        const resProjects = await fetch(`${API_BASE}/api/lgu/projects?uid=${user.uid}`);
        if (resProjects.ok) {
